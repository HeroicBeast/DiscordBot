...
const run = async (client, message, args) => {
	let img = await new e.DoubleStonk().getImage(
		client.user.displayAvatarURL({ format: 'png', dynamic: false }),
		message.author.displayAvatarURL({ format: 'png', dynamic: false })
	);
	let attch = new discord_js_1.default.MessageAttachment(img, 'Slap.png');
	if (!args.length) return message.channel.send(attch);
	let member =
		message.mentions.members.first() ||
		message.guild.members.cache.get(args[0]);
	if (!member) return message.channel.send(attch);
	let img2 = await new e.DoubleStonk().getImage(
		message.author.displayAvatarURL({ format: 'png', dynamic: false }),
		member.user.displayAvatarURL({ format: 'png', dynamic: false })
	);
	let attch2 = new discord_js_1.default.MessageAttachment(img2, 'Slap.png');
	message.channel.send(attch2);
};
exports.run = run;
exports.name = 'doublestonks';
exports.aliases = ['dstonks'];
exports.category = 'Image';
exports.cooldown = 5000;
exports.description = 'Slap someone';
exports.usage = 'slap <mention or id>';
//# sourceMappingURL=DoubleStonks.js.map
