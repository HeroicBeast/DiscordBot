...
const run = async (client, message, args) => {
	let img = await new e.Triggered().getImage(
		message.author.displayAvatarURL({ format: 'png', dynamic: false })
	);
	let attch = new discord_js_1.default.MessageAttachment(img, `Trigger.gif`);
	if (!args.length) return message.channel.send(attch);
	let member =
		message.mentions.members.first() ||
		message.guild.members.cache.get(args[0]);
	if (!member) return message.channel.send(attch);
	let img2 = await new e.Triggered().getImage(
		member.user.displayAvatarURL({ format: 'png', dynamic: false })
	);
	let attch2 = new discord_js_1.default.MessageAttachment(img2, `Trigger.gif`);
...
exports.description = "Get a Triggered Gif Of Someone's Avatar";
exports.usage = 'trigger <mention or id>';
