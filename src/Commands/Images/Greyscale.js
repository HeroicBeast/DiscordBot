...
const run = async (client, message, args) => {
	let img = await new e.Greyscale().getImage(
		`${message.author.displayAvatarURL({
			format: 'png',
			dynamic: false,
		})}?size=256`
	);
	let attch = new discord_js_1.default.MessageAttachment(img, 'Greyscale.png');
	if (!args.length) return message.channel.send(attch);
	let member =
		message.mentions.members.first() ||
		message.guild.members.cache.get(args[0]);
	if (!member) return message.channel.send(attch);
	let img2 = await new e.Greyscale().getImage(
...
exports.run = run;
exports.name = 'grey';
exports.aliases = ['gray'];
exports.category = 'Image';
exports.cooldown = 5000;
exports.description = "Make Someone's Avatar Grey";
exports.usage = 'grey <mention or id>';
