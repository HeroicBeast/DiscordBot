...
const run = async (client, message, args) => {
	let img = await new e.Invert().getImage(
		`${message.author.displayAvatarURL({
			format: 'png',
			dynamic: false,
		})}?size=256`
	);
	let attch = new discord_js_1.default.MessageAttachment(img, 'Invert.png');
	if (!args.length) return message.channel.send(attch);
	let member =
		message.mentions.members.first() ||
		message.guild.members.cache.get(args[0]);
	if (!member) return message.channel.send(attch);
	let img2 = await new e.Invert().getImage(
		`${member.user.displayAvatarURL({
			format: 'png',
			dynamic: false,
		})}?size=256`
	);
	let attch2 = new discord_js_1.default.MessageAttachment(img2, 'Invert.png');
...
exports.description = "Invert Someone's Avatar";
exports.usage = 'invert <mention or id>';
